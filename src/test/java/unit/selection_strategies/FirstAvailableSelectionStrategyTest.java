...
public class FirstAvailableSelectionStrategyTest {

    BackendSelectionStrategy firstAvailable = new FirstAvailableSelectionStrategy();
    ...
    public void selectOneBackend() {
        ...
        for (Backend backend : mockBackends) {
            if (backend.isHealthy()) {
                assertEquals(backend.getAddress(), firstAvailable.select(mockBackends).getAddress());
                break;
            }
        }
    }
